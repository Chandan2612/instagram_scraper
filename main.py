...
# #         page.wait_for_timeout(4000)  # 🔥 important (wait for HTML load)
...
# #             time.sleep(5)  # 🔥 important (meta load)
...
#     """Press ArrowDown and wait until the reel URL actually changes."""
#     start = time.time()
...
#             current_reel_url = page.url  # 🔑 Capture BEFORE scraping
#             username = get_username_from_page(page)
...
#             if profile_url in visited:
#                 print(f"⏭ Already visited: {username} → forcing reel change")
#                 # 🔑 KEY FIX: wait for actual URL change, not just sleep
#                 wait_for_reel_change(page, current_reel_url)
#                 continue
...
#             # 🔑 After going back, wait for URL to settle on a reel
#             back_url = page.url
#             wait_for_reel_change(page, back_url)
